...
Security hardening is now moving through the DAL-169/DAL-172 sequence. `DAL-169` is merge-ready after production route/webhook/PII/replay hardening and broader regression. `DAL-170A` added the first hard tenant-isolation audit and regression baseline: a read-only Neon/Postgres audit script plus cross-tenant API tests for lead detail/list, hot list, buyer cards, offers, drafts, viewings, media metadata, share/reassign, same-phone buyer profiles, and lead-ingest tenant attribution. `DAL-170B` closed the highest-risk externally reachable listing-route slice: legacy listing activate / portal links / stats / conversations / media endpoints are admin-gated, and `/parse-spa` is now read-only plus active-brokerage-member gated so it no longer leaks global stable-listing existence or writes tenantless listings. `DAL-170C` first pass is merge-ready too: it adds dry-run tenant-root normalization visibility, null-tenant fail-closed guards in active APIs, and legacy buyer-profile quarantine behavior without backfills or RLS. `DAL-170C` second-pass tooling now exists for reviewed `listing_inquiries` and `offer_records` backfills, but the shared `.env.test` branch has already been normalized by apply validation, so the earlier 53/47 counts are stale and the real apply now needs either a fresh branch or a re-approved baseline. `DAL-170D1` is merge-ready as the first additive database-integrity layer: it adds tenant-constraint preflight, production-blocked DDL tooling, safe non-partial parent composite indexes, child supporting indexes, and first NOT VALID tenant FKs on `.env.test` without data writes, RLS, NOT NULL, or constraint validation. `DAL-170D2` is merge-ready as the second additive FK slice: it adds only listing-linked NOT VALID tenant FKs for `offers`, `draft_replies`, `viewings`, and `media_assets` to `listings(brokerage_id, listing_id)` on `.env.test`. `DAL-170D-PROD` is merge-ready as guardrail tooling only: future production/staging DDL now requires non-secret DB fingerprint confirmation, artifact bundles, one-phase execution, rollback SQL, timeout settings, evidence-gated rehearsal overrides, and the runbook in `docs/runbooks/dal170d-production-ddl.md`. A direct-URL dedicated Neon rehearsal branch successfully applied all DAL-170D1/D2 phases and catalog verification showed all 9 tenant FKs still `convalidated=false`; after fixing the DAL-170C normalization subprocess harness to honor `DALYA_TEST_ENV_FILE`, combined security + tenant regression and broader regression both passed on that rehearsal branch. DAL-170D1/D2 DDL has now also been applied to staging only, one phase at a time, with 7 parent indexes, 15 child indexes, and all 9 NOT VALID tenant FKs present and still `convalidated=false`; staging combined security + tenant regression and broader regression both passed. Production DDL was not run and is intentionally held during the current build/tinkering phase because no official brokerage has been onboarded, no real brokerage data has been imported, and no live WhatsApp buyer traffic is connected to production. Revisit DAL-170D production rollout before onboarding Luqman's agency, importing real brokerage data, or connecting live WhatsApp buyer traffic. `DAL-172A` closed the known backend multi-brokerage ambiguity: authenticated agent/dashboard/listing/viewing/SPA routes accept `X-Brokerage-Id`, preserve single-membership fallback, and fail closed for multi-membership users without explicit context; listing creation also validates explicit managing-agent assignment against same-brokerage active membership and managing-role rules. `DAL-172B` adds the frontend side of that contract: the app shell fetches safe active memberships from `GET /api/v1/me/brokerages`, auto-selects only a single active brokerage, requires explicit selection for multi-brokerage users, persists valid selection locally, clears stale/forbidden selections, centrally sends `X-Brokerage-Id` only for authenticated brokerage-scoped API calls, and makes `/api/v1/onboarding/me` selector-aware so visible onboarding status does not silently choose a first active membership. The audit still intentionally exposes unresolved work rather than fixing it: nullable or null tenant ownership remains in legacy/core tables, several high-risk child tables derive tenant through parents, and `listings.listing_id` remains globally unique until later DAL-170 passes.
