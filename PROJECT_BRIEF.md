...
*Last updated: 2026-06-23 (DAL-198 verification closeout cleanup)*
...
Security hardening is now moving through the DAL-169/DAL-172 sequence. `DAL-169` is merge-ready after production route/webhook/PII/replay hardening and broader regression. `DAL-170A` added the first hard tenant-isolation audit and regression baseline: a read-only Neon/Postgres audit script plus cross-tenant API tests for lead detail/list, hot list, buyer cards, offers, drafts, viewings, media metadata, share/reassign, same-phone buyer profiles, and lead-ingest tenant attribution. `DAL-170B` closed the highest-risk externally reachable listing-route slice: legacy listing activate / portal links / stats / conversations / media endpoints are admin-gated, and `/parse-spa` is now read-only plus active-brokerage-member gated so it no longer leaks global stable-listing existence or writes tenantless listings. `DAL-170C1/C2/C3` are now split and merged: C1 covers tenant normalization audit/dry-run tooling, C2 covers runtime null-tenant and buyer-profile quarantine guardrails, and C3 stops portal lead ingest from writing active buyer-source state into the legacy/global phone-keyed buyer profile table. `DAL-170D1/D2/PROD` added production-blocked tenant-constraint preflight, safe parent/child index tooling, NOT VALID tenant FK phases, DB fingerprint/artifact/rollback guardrails, and the production DDL hold decision; the DDL rehearsal and staging rollout passed, but production DDL remains intentionally held until official onboarding / real production data. `DAL-172A/B` closed explicit brokerage context end to end: backend routes validate `X-Brokerage-Id` and fail closed for multi-brokerage users without context, while the frontend requires explicit brokerage selection before authenticated brokerage-scoped API calls. `DAL-170E1/E2/E3/E4A/E4B/E4C` now prove staged, rehearsal-only RLS and app-role readiness without enabling production/staging RLS: SQLAlchemy sessions store request/service DB context, E1/E2 cover direct-root tenant tables, E3 adds rehearsal policies for high-risk derived/no-root or nullable-root tables while keeping `message_queue` and legacy/global `buyer_profiles` service-only, E4A gates runtime schema creation so production/staging app startup cannot attempt `Base.metadata.create_all()` under a least-privilege app role, E4B adds guarded test/local rehearsal tooling for a non-owner no-`BYPASSRLS` app runtime role with DML/sequence/schema grants, and E4C makes webhook, worker, health, signed-media, research/admin, CRM-admin, and resolver-owned sessions set explicit service/admin DB context. `DAL-171A/B/C1/C2/C3` close the first private-media slice: outbound local media uses short-lived HMAC-signed retrieval URLs instead of an unauthenticated static mount, inbound buyer provider media is re-hosted as tenant-scoped `DBMediaAsset` rows with durable metadata pointing at `media_asset_id` rather than raw Twilio/provider URLs, local media has per-asset signing-nonce revocation plus revoked/deleted/retention lifecycle state, the PDPL deletion helper can revoke/delete buyer-linked media bytes while redacting durable media metadata and preserving non-PII compliance evidence, and external listing/document/media URLs now fail closed unless they are public `https` references. No broad all-table RLS, production/staging RLS, production DDL execution, production/staging role changes, data backfill, chatbot behavior expansion, WhatsApp send behavior change, frontend behavior, or Supabase Storage migration changed in this slice. Remaining production RLS work is staged: production rollout gates come after app-role and service/admin compatibility rehearsal.
