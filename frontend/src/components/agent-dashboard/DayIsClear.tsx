...
export function DayIsClear({
  ...
  const showsFirstRunActivation = Boolean(emptyState)
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-1 text-xl font-semibold tracking-tight text-neutral-900">
            {showsFirstRunActivation ? 'Start with an internal pilot rehearsal' : 'Your day is clear'}
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-neutral-600">
            {showsFirstRunActivation
              ? 'This authenticated workspace has no live buyer activity yet. Use synthetic/internal records only until Eric approves the pilot users, WhatsApp numbers, and data class.'
              : 'No urgent buyer replies, hot signals, drafts, viewings, or escalations are waiting right now.'}
          </p>
        </div>
      </div>
